...
func main() {
	...
	// 使用公钥加密数据
	encrypted, err := rsa.EncryptOAEP(sha256.New(), rand.Reader, &privateKeyRSA.PublicKey, message, s)
	if err != nil {
		fmt.Println(err)
		panic(err)
	}
	...
	// 使用私钥解密数据
	decrypted, err := rsa.DecryptOAEP(sha256.New(), rand.Reader, privateKeyRSA, encrypted, s)
	if err != nil {
		fmt.Println(err)
		panic(err)
	}
